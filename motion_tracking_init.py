...
def handleMouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ix, iy = x, y
        print("draw started. ix=", x, ", iy=", y)
    elif event == cv2.EVENT_LBUTTONUP:
        ix2 = x
        iy2 = y
        print("draw ended. ix2=", x, ", iy2=", y)
    ...
    if show_livefeed:
        ...
        if not thresh is None:
            ...
            for c in cnts:
                ...
                cv2.rectangle(
                    frame1, (ix + x, iy + y), (ix + x + w, iy + y + h), (0, 255, 0), 2
                )
        ...
        if threshold >= 0:
            ...
            if globalmotion_flags.count(1) > globalmotion_flags_limit / 2:
                ...
                if not globalmotion_status == 1:
                    ...
                    # toast.show_toast("Machine "+targetcam, "Machine just started running again", duration = 3, icon_path = "config/python_icon.ico")
                    globalmotion_lastchange_date = datetime.datetime.now()
                ...
                if not globalmotion_status == 0:
                    ...
                    # toast.show_toast("Machine "+targetcam, "Machine just stopped", duration=3, icon_path="config/python_icon.ico")
                    globalmotion_msg = (
                        "Not Moving since "
                        + datetime.datetime.now().strftime("%Y/%m/%d %H:%M:%S")
                    )
                    ...
                    if (
                        minutes_from_first_stop >= globalmotion_mins_to_alert_onstop
                        and not globalmotion_stopalert_displayed
                    ):
                        globalmotion_stopalert_displayed = True
                        toast.show_toast(
                            "Alert - Machine " + targetcam,
                            "Machine was stopped for "
                            + str(globalmotion_mins_to_alert_onstop)
                            + " mins",
                            duration=3,
            ...
            str2 = (
                ...
                + str(freeze_frames)
                + "("
                + str(freeze_frames_perc)
                + "%)"
            )
            cv2.putText(
                frame1, str1, (10, 70), font, 0.5, (255, 255, 255), 1, cv2.LINE_AA
            )
            cv2.putText(
                frame1, str2, (10, 90), font, 0.5, (255, 255, 255), 1, cv2.LINE_AA
            )

        thresh_str = "Thresholding @ =" + str(threshold)
        status_str = "Current Status=" + str(globalmotion_msg)
        cv2.putText(
            frame1, thresh_str, (10, 110), font, 0.5, (255, 255, 255), 1, cv2.LINE_AA
        )
        cv2.putText(
            frame1, status_str, (10, 130), font, 0.5, (255, 255, 255), 1, cv2.LINE_AA
        )
...
toast.show_toast(
    "WOW!!", "script completed", duration=3, icon_path="config/python_icon.ico"
)
